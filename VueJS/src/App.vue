    ...
    export default {
        ...
        components: {
            ...
            this.$root.$on("onRegister", (login, name) => {
                ...
                if (!login || login.length < 3 || login.length > 16 || flag) {
                    this.$root.$emit("onRegisterValidatorError", "Login must contain 3-16 latin letters");
                } else if (!name || name.length > 32) {
                    this.$root.$emit("onRegisterValidatorError", "Name must contain 1-32 character");
                } else {
                    if (Object.values(this.users).filter(u => u.login === login).length) {
                        this.$root.$emit("onRegisterValidatorError", "Login is already in use");
                    } else {
